...
namespace WebApplication1.Services {
    public class FilesServices {
        ...
        public static async Task<byte[]> GetFileBytes(string filePath) {
            try {
                byte[] buffer;
        ...
        public static List<Files> GetAll(WhereSQL whereSQL) {
            try {
        ...
        public static Files? GetSingle(WhereSQL whereSQL) {
            try {
                using (SQLiteConnection dbContext = DBContext.GetInstance()) {
                    using (SQLiteCommand command = new SQLiteCommand($"SELECT * FROM {FilesSQLTable.tableName} {whereSQL.GetClausule()}",dbContext)) {
                        using (SQLiteDataReader reader = command.ExecuteReader()) {
